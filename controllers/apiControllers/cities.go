...
func cities(countrykey string) interface{}{
	...
	if countrykey != "" {
		iter := cassandraSession.Query("SELECT city_id, city, citykey, count, country, country_id, countryisocode, countryisocode_lower, countrykey FROM cities_by_country WHERE countrykey = '" + countrykey + "' ORDER BY count DESC LIMIT 16").Iter()
		i := 0
